...
//var component = require(`${PROJECT_DIR}/controllers/invoice-line-items/invoice-line-item.component`);
 ...
 */
var fields = [`sfid`,`amount__c`,`name`,`invoice_number__c`,`item__c`,`date_part('epoch'::text, ordered_date__c) * (1000)::double precision as ordered_date__c`,`quantityinvoiced__c`,`date_part('epoch'::text, createddate) * (1000)::double precision as createddate`];
async function getAll(req) {
    try {

        is_Validate = true;
        is_Validate = is_Validate ? validation.issetNotEmpty(req.headers.agentid) : false;
        is_Validate = is_Validate ? validation.issetNotEmpty(req.headers.invoiceid) : false;
       
        if (is_Validate) {


         
            const tableName = 'Invoice_Line__c';

            const WhereClouse = [];
            var offset = '0', limit = '1000';
            if (validation.issetNotEmpty(req.query.invoiceid)) {
                    WhereClouse.push({ "fieldName": "invoice_number__c", "fieldValue": req.query.invoiceid });
            }
            ...
            if (orders.rowCount != undefined && orders.rowCount > 0) {
                response.response = { 'success': true, "data": { "invoice-line-items": orders.rows } };
                response.status = 200;
                return response;
            } else {
                response.response = { 'success': false, "data": { "invoice-line-items": [] }, "message": "No record found." };
                response.status = 400;
                return response;
            }
        } else {
            response.response = { 'success': false, "data": { "invoice-line-items": [] }, "message": "Mandatory parameter(s) are missing." };
            response.status = 400;
            return response;
        }
    } catch (e) {
        console.log(e);
        response.response = { 'success': false, "data": { "invoice-line-items": [] }, "message": "Internal server error." };
        response.status = 500;
        return response;
    }
}
...
async function detail(req) {
    ...
    try {
        ...
        if (is_Validate) {
            ...
            let sql = `SELECT `;
            sql += fields.join(',');
            sql += ` FROM ${process.env.TABLE_SCHEMA_NAME}.Invoice_Line__c where sfid='${req.query.id}' and isdeleted='false'`;
            sql += ` limit 1`;
